...
### Provider Routing
Manually selecting modules from multiple providers. This helps solve vendor lock-in and improves reliability. This is often the *same model* (e.g. Llama 4) but hosted by different providers (Groq, Together, Replicate).
...
The real power of this approach comes from its adaptability. When a new model like GPT-5.5 drops, you don't need to rewrite thousands of heuristic rules or retrain a binary classifier. You simply run the new model against a sample of requests from each semantic cluster. If it outperforms the incumbent in the "SQL optimization" cluster but fails in the "creative poetry" cluster, the routing map updates automatically. The system learns the contours of the new model's intelligence profile instead of relying on a human's intuition about what it "should" be good at.
