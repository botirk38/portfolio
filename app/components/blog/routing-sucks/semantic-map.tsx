...
export const SemanticMap = () => {
  ...
  const points = [
    { x: 20, y: 30, label: "Creative Writing", model: "Claude 4.5 Opus", color: "bg-purple-500" },
    { x: 70, y: 20, label: "React Code", model: "Claude 4.5 Sonnet", color: "bg-blue-500" },
    { x: 50, y: 60, label: "SQL Queries", model: "GPT-5.2", color: "bg-green-500" },
    { x: 80, y: 80, label: "Math Proofs", model: "Gemini 3 Pro", color: "bg-red-500" },
    { x: 30, y: 80, label: "Data Extraction", model: "Llama 4", color: "bg-yellow-500" },
  ];
      ...
      <div className="relative h-[300px] w-full bg-white dark:bg-black rounded border border-neutral-200 dark:border-neutral-800 overflow-hidden">
        ...
        <div className="absolute inset-0 grid grid-cols-6 grid-rows-6 pointer-events-none">
          {Array.from({ length: 36 }).map((_, i) => (
            <div key={i} className="border-[0.5px] border-neutral-100 dark:border-neutral-900" />
          ))}
        </div>
        ...
        {points.map((point, i) => (
          ...
          >
            ...
            >
              <div className={`w-4 h-4 rounded-full ${point.color} shadow-lg ring-4 ring-white/20 dark:ring-black/20`} />
              
              {/* Pulse effect */}
              <div className={`absolute inset-0 -z-10 w-4 h-4 rounded-full ${point.color} animate-ping opacity-20`} />
              ...
              `}>
                <div className="flex flex-col items-center gap-0.5">
                  ...
                  <span className="opacity-70 text-[10px]">Best: {point.model}</span>
                </div>
                {/* Arrow */}
